...
parser.add_argument('--fg_path', type = str, required = True,
                    help = 'path to provided foreground images')
parser.add_argument('--mask_path', type = str, required = True,
                    help = 'path to provided alpha mattes')
parser.add_argument('--bg_path', type = str, required = True,
                    help = 'path to to background images (MSCOCO)')
parser.add_argument('--out_path', type = str, required = True,
                    help = 'path to folder where you want the composited images to go')

parser.add_argument('--out_csv', type = str, default = os.devnull,
                    help = 'path to csv file used by data loader')
parser.add_argument('--num_bgs', type = int, default = 100,
                    help = 'number of backgrounds onto which to paste each foreground')
parser.add_argument('--workers', type = int, default = 1,
                    help = 'maximum workers to use, defaults to 1')
...
def process_foreground_image(i, job):
    ...
    for b, bg_name in enumerate(pbar):
        ...
        try:
            ...
            line = (
                f'{os.path.join(fixpath(fg_path), im_name)};'
                f'{os.path.join(fixpath(a_path), im_name)};'
                f'{fixpath(out_name)};'
                f'{fixpath(back_name)}\n'
            )
