...
def align_images(im1, im2, masks_dl):
    ...
    img_aligned = cv2.warpPerspective(im1, h, (width, height))

    # copy image in the empty region, unless it is a foreground. Then copy background
    mask_rep = (np.sum(img_aligned.astype('float32'), axis=2) == 0)

    img_aligned[mask_rep, 0] = im2[mask_rep, 0]
    img_aligned[mask_rep, 1] = im2[mask_rep, 1]
    img_aligned[mask_rep, 2] = im2[mask_rep, 2]

    mask_rep1 = np.logical_and(mask_rep, masks_dl[..., 0] == 255)

    img_aligned[mask_rep1, 0] = im1[mask_rep1, 0]
    img_aligned[mask_rep1, 1] = im1[mask_rep1, 1]
    img_aligned[mask_rep1, 2] = im1[mask_rep1, 2]

    return img_aligned


def adjust_exposure(img, back, mask):
...
for i in range(0, len(list_im)):
    ...
    back_align = align_images(back, image, mask)
